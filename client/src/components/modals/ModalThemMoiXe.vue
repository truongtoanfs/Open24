<template>
    <base-modal modalSize="lg" requiredTwoCols @closeModal="$emit('closeModal')">
        ...
        <template v-slot:modal-content>
            ...
            <base-input-group-select requiredPlus class="mb-2.5 xl:order-3" @openModalAddNew="$emit('openModalMauXe')" label="Mẫu xe" placeholderText="Chọn mẫu xe" :value="updateXeData ? updateXeData.TenMauXe : ''" :selectList="mauXeList"/>
            <base-input-group-select requiredPlus class="mb-2.5 xl:order-5" @openModalAddNew="$emit('openModalHangXe')" label="Hãng xe" placeholderText="Chọn hãng xe" :value="updateXeData ? updateXeData.TenHangXe : ''" :selectList="hangXeList" />
            <base-input-group-select requiredPlus class="mb-2.5 xl:order-7" @openModalAddNew="$emit('openModalLoaiXe')" label="Loại xe" placeholderText="Chọn loại xe" :value="updateXeData ? updateXeData.TenLoaiXe : ''" :selectList="loaiXeList" />
            <base-input-group class="mb-2.5 xl:order-9" label="Năm SX" :value="updateXeData ? updateXeData.NamSanXuat : ''" />
            <base-input-group-select-info requiredPlus class="mb-2.5 xl:order-11" @openModalAddNew="$emit('openModalKhachHang')" label="Chủ xe" placeholderText="Khách hàng" :value="updateXeData ? updateXeData.TenLoaiXe : ''" :selectList="chuXeList" />
